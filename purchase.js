function PrintProducts() {
  ...
  function MinusItem(itemName) {
    ...
    console.log("minus", userbasket);
  }
  
  function Purchase() {
    const total = 0;
    for (let i = 0; i < products.length; i++) {
      for (let j = 0; i < userbasket.length; i++) {
        if (userbasket[i].name === products[i].name) {
          total += products[i].price * userbasket[j].quantity;
        }
      }
    }
